...
export class OwnerViewComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.bakeryOwnerProfileInfoSubscription =
      this.profileService.bakeryOwnerProfileInfo$.subscribe();
  }
  ...
  getImageUrl(profileImage: string | null) {
    ...
    if (profileImage?.startsWith('data:image/png')) {
      ...
      profileImage?.startsWith('data:image/jpeg') ||
      profileImage?.startsWith('data:image/jpg')
    ) {
      imageType = 'jpeg';
    }
